...
// of the second — they take their wrong tiles straight from `nearestToAny`.

/**
 * Candidates nearest any of the targets in frequency rank, closest first.
 *
 * A sentence is several words at once, and its wrong tiles should be plausible
 * beside all of them rather than beside the first. Distance is to the *nearest*
 * target, not to their average: a line pairing 我 with a word from four years
 * later has no midpoint, and the words either side of each end are better tiles
 * than anything in the gap.
 *
 ...
 */
export function nearestToAny(
  targets: readonly string[],
  candidates: readonly string[],
  rankOf: (headword: string) => number | undefined,
  count: number,
): string[] {
  // A word offered as its own distractor makes two options correct, and a tile
  // that is already in the answer makes the check ambiguous. Both are the one
  // way a set can be wrong rather than merely easy.
  const targeted = new Set(targets)
  const pool = [...new Set(candidates)].filter((word) => !targeted.has(word))

  const anchors = targets.map(rankOf).filter((rank) => rank !== undefined)
  if (!anchors.length) return pool.slice(0, count)

  const ranked: Array<{ word: string; rank: number; distance: number }> = []
  const unranked: string[] = []
  for (const word of pool) {
    const rank = rankOf(word)
    if (rank === undefined) unranked.push(word)
    else ranked.push({ word, rank, distance: Math.min(...anchors.map((a) => Math.abs(rank - a))) })
  }
  ...
  ranked.sort((a, b) => a.distance - b.distance || a.rank - b.rank || (a.word < b.word ? -1 : 1))
...
/**
 * The same, for a question with one right answer.
 *
 * A word card and a grammar card each have exactly one thing being asked about,
 * so this is `nearestToAny` with a single anchor rather than a second ranking —
 * `.claude/rules/llm-and-asr.md` says one ranking per language, and the
 * argument holds a level down as well.
 */
export function nearestByRank(
  ...
  return nearestToAny([target], candidates, rankOf, count)
}
