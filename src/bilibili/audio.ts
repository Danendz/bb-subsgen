 ...
 *
 * Called from the content script, though not because it has to be:
 * `api.bilibili.com` applies no referrer check, unlike the media CDN that
 * `fetchAudioBytes` exists for. Where this one runs is a question of who ought
 * to know about Bilibili rather than of what is permitted, and the answer is
 * the module named after it.
 *
 * Re-called rather than remembered: a
